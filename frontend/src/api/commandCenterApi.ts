import type { Agent, Approval, AuditLog, ConsoleSnapshot, DashboardData, GlobalSearchResult, McpServer, McpTool, Mission, OfficeLayout, OfficeState, Skill, SystemSetting, Task, TaskRun, WorkflowTemplate, Workspace } from '../types/domain';
...
export const commandCenterApi = {
  ...
  startMission: (missionId: string, actorName: string) => apiRequest<{ mission: Mission; task: Task; requiresApproval: boolean }>(`/missions/${missionId}/start`, { method: 'POST', body: JSON.stringify({ actorName }) }),
  pauseMission: (missionId: string, actorName: string) => apiRequest<Mission>(`/missions/${missionId}/pause`, { method: 'POST', body: JSON.stringify({ actorName }) }),
  resumeMission: (missionId: string, actorName: string) => apiRequest<Mission>(`/missions/${missionId}/resume`, { method: 'POST', body: JSON.stringify({ actorName }) }),
  cancelMission: (missionId: string, actorName: string) => apiRequest<Mission>(`/missions/${missionId}/cancel`, { method: 'POST', body: JSON.stringify({ actorName }) }),
  ...
  launchWorkflowTemplate: (workflowId: string, payload: { createdBy: string; sandbox: boolean; workspaceId?: string }) => apiRequest<Mission>(`/workflows/${workflowId}/launch`, { method: 'POST', body: JSON.stringify(payload) }),
  getWorkspaces: () => apiRequest<Workspace[]>('/workspaces'),
  createWorkspace: (payload: { name: string; slug: string; description: string; ownerName: string; ownerEmail: string }) => apiRequest<Workspace>('/workspaces', { method: 'POST', body: JSON.stringify(payload) }),
  updateWorkspace: (workspaceId: string, payload: { actorName: string; name: string; slug: string; description: string; status: string }) => apiRequest<Workspace>(`/workspaces/${workspaceId}`, { method: 'PUT', body: JSON.stringify(payload) }),
  createWorkspaceMembership: (workspaceId: string, payload: { actorName: string; displayName: string; email: string; roleKey: string }) => apiRequest(`/workspaces/${workspaceId}/memberships`, { method: 'POST', body: JSON.stringify(payload) }),
